...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            downloadButton("downloadData2", "Download as JSON")
        ),
        
        # Show a plot of the generated distribution
        mainPanel(
            fluidRow(
                column(6, plotOutput("distReal")), column(6,plotOutput("distPlot")),
...
server<- function(input,output,session){
	...
	#	Generation of VC
	#	Size and Demographics generation
        ...
        Age_per_gender <- ifelse(Age_per_gender < 18, 18, Age_per_gender)	# correct ages below 18 (not recruitment allowed)
	...
	# Generation of non-demographic numerical variables (lab values) 
	    
        # Below commented how correlations has been collected from real clinical data (test_covid)        
	...
	# corrections for some variable
	    
        VC_375_data <- output
	    VC_375_data <- output %>% mutate(albumin = ifelse(outcome == 0, 2.5*albumin, albumin),
                                         Prothrombin_activity = ifelse(outcome == 0, 2.5*Prothrombin_activity, Prothrombin_activity),
                                         percent_lymphocyte = ifelse(outcome == 0, 3*percent_lymphocyte, percent_lymphocyte))

        
        rm(patient);rm(patient2);rm(datarnd);rm(cdatarnd);rm(output) #cleaning heavy environment
        
        VC_375_data_KM <- VC_375_data %>%
            group_by(PATIENT_ID) %>% 
    ...
    #Kaplan-Meier analysis as demonstrators
    ...
    # Hospitalization-day density plot
    ...
    output$distPlot <- renderPlot(
        ...
        test2() %>%
            ...
            guides(fill=guide_legend(title="Outcome 0=living 1=dead")) +
            xlab("Hospitalization days") +
    ...
    output$distReal <- renderPlot(
        
        test_covid %>%
            group_by(PATIENT_ID) %>% 
            mutate(hospitalization = as.numeric(Discharge_time - Admission_time, units = "days")) %>% 
            ungroup() %>% 
            select_at(c("PATIENT_ID","hospitalization", "gender", "outcome")) %>% 
            ggplot() +
            ...
            scale_x_continuous(breaks=seq(0, 50, 5))+
            theme(legend.position="bottom") +
            ggtitle("Distribution of Hospitalization days for a real COVID-19 Cohort (n=375)")
    ...
    output$downloadData <- downloadHandler(
        ...
        filename = function(){
            a <- paste("SASC_output_",format(Sys.time(), "%b_%d_%X"),".csv", sep = "")
            gsub(":", "", a)
        },
        
        content = function(file) {
            write.csv(test3(), file, row.names = FALSE)
            #write_json(as.list(test3()),gsub("csv","json",file))
        }
        
    )
    
    # Downloadable JSON of selected dataset ----
	
    output$downloadData2 <- downloadHandler(
        
        filename = function(){
            a <- paste("SASC_output_",format(Sys.time(), "%b_%d_%X"),".json", sep = "")
            gsub(":", "", a)
        },
        
        content = function(file) {
            #write.csv(test3(), file, row.names = FALSE)
            write_json(as.list(test3()),file)
        }
